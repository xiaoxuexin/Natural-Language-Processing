...
def load_embeddings(config, emb_type='wiki', emb_file_name=None):
    if emb_type == 'new':
        ...
        # TODO
        model_twitter = gensim.models.KeyedVectors.load_word2vec_format('twitter.word2vec.100-iter.bin', binary=True,
                                                                unicode_errors='ignore')
        tw_tens = torch.tensor(model_twitter.vectors)
        tw_embedding = nn.Embedding(model_twitter.vectors.shape[0], model_twitter.vectors.shape[1])
        tw_embedding.weight = nn.Parameter(tw_tens)
        tw_embedding.weight.requires_grad = False
        word_embeddings = tw_embedding
        pos_embeddings = nn.Embedding(config.pos_vocab_size, config.embedding_dim)
        dep_embeddings = nn.Embedding(config.dep_vocab_size, config.embedding_dim)
        pass
    elif emb_type == 'wiki' or emb_type == 'wikipedia':
        # TODO
        model_wiki = gensim.models.KeyedVectors.load_word2vec_format('wiki.word2vec.min-100.bin', binary=False,
                                                                        unicode_errors='ignore')
        wk_tens = torch.tensor(model_wiki.vectors)
        wk_embedding = nn.Embedding(model_wiki.vectors.shape[0], model_wiki.vectors.shape[1])
        wk_embedding.weight = nn.Parameter(wk_tens)
        wk_embedding.weight.requires_grad = False
        word_embeddings = wk_embedding
        pos_embeddings = nn.Embedding(config.pos_vocab_size, config.embedding_dim)
        dep_embeddings = nn.Embedding(config.dep_vocab_size, config.embedding_dim)
        pass
    else:
        raise RuntimeError('unknown embedding type!: "%s"' % emb_type)

    return word_embeddings, pos_embeddings, dep_embeddings


def train(save_dir='saved_weights', parser_name='parser', num_epochs=10, max_iters=-1,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i, (train_x, train_y) in enumerate(get_minibatches([dataset.train_inputs,
            ...
            # function in numpy.
            # print("********************")
            # labels = [np.argmax(i) for i in train_y]# TODO
