...
class ParserModel(nn.Module):
    ...
    def __init__(self, config, word_embeddings=None, pos_embeddings=None,
        ...
        self.in_features = (n_d + n_p + n_w) * self.config.embedding_dim
        self.out_features = self.config.l1_hidden_size
        self.first_layer = nn.Linear(self.in_features, self.out_features)
        # TODO
        ...
        self.drop_layer = nn.Dropout(p = self.config.keep_prob)
        # TODO
                
        # Create the output layer that maps the activation of the hidden layer to
        # the output classes (i.e., the valid transitions)
        self.output_layer = nn.Linear(self.out_features, self.config.num_classes)
        # TODO
    ...
    def init_weights(self):
        ...
        # faster convergence.
        initrange = 0.1
#        ww1 = torch.Tensor(self.in_features, self.out_features)
#        self.w1 = nn.init.uniform(ww1) * 2 * initrange - initrange
#        ww2 = torch.Tensor(self.out_features, self.config.num_classes)
#        self.w2 = nn.init.uniform(ww2) * 2 * initrange - initrange
    ...
    def lookup_embeddings(self, word_indices, pos_indices, dep_indices, keep_pos = 1):
        ...
        # word_tens = torch.Longtensor(word_indices)
        w_embeddings = self.word_embeddings(word_indices)
        # pos_tens = torch.tensor(pos_indices)
        p_embeddings = self.pos_embeddings(pos_indices)
        # dep_tens = torch.tensor(dep_indices)
    ...
    def forward(self, word_indices, pos_indices, dep_indices):
        ...
        # useful here.        
        w_emb = w_embeddings.view(w_embeddings.shape[0], -1)
        p_emb = p_embeddings.view(p_embeddings.shape[0], -1)
        d_emb = d_embeddings.view(d_embeddings.shape[0], -1)
        input = torch.cat((w_emb, p_emb, d_emb), 1)
        # print("the catencate flattened tensor size ", input.size())
        # TODO
        ...
        first_output = self.first_layer(input)
        # TODO
        ...
        temp_tens = torch.pow(first_output, 3)
        # TODO
        

        # Now do dropout for final activations of the first hidden layer
        inter_output = self.drop_layer(temp_tens)
        # TODO
        ...
        output = self.output_layer(inter_output)
        # TODO
