...
class Sampler:
    ...
    def add_doc(self, doc, vocab, doc_id = None,
        ...
        for ww in temp_doc:
            ...
            # print("*******************")
            # print(self._topics._topic_term[assignment][ww])
            token_count += 1
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        if old_topic != -1:
            ...
            # assignments
            assign = self._doc_assign[doc][index]

            self._doc_assign[doc][index] = -1

            self._doc_counts[doc][assign] -= 1
            # self._topics._topic_term[assign][term] -= 1
            self._topics.change_count(assign, term, -1)
            # print("***************")
            # print(self._topics._topic_term[assign][term])
        if new_topic != -1:
            assert old_topic == -1
    ...
    def report_topics(self, vocab, outputfilename, limit=10):
        ...
        mytopicfile = open("my-topics.txt", 'w')
        for keys in self._topics._topic_term.keys():
            mytopicfile.write("------------\nTopic %i \n------------\n" % keys)
            l = []
            vocab_list = []
            for i in self._topics._topic_term[keys]:
                vocab_list.append(i)
                l.append(self._topics._topic_term[keys][i])
            arr = np.array(l)
            ind_arr = arr.argsort()[-100:][::-1]

            listt = [vocab[ind] for ind in ind_arr]
            tempp = ""
            for i in range(len(listt)):
                tempp += str(listt[i]) + " "
            tempp += "\n"
            mytopicfile.write(tempp)
        mytopicfile.close()

        for i in self._topics._topic_term.keys():
            print(self._topics._topic_term[i])
    ...
    def sample_probs(self, doc_id, index):
        ...
        for kk in range(self._num_topics):
            ...
            beta = 0.01
            topic = self._doc_assign[doc_id][index]
            n_d_k = self._doc_counts[doc_id][topic]
            part1 = (n_d_k + self._alpha[kk]) / (sum(self._doc_counts[doc_id].values()) + sum(self._alpha))
            word = self._doc_tokens[doc_id][index]
            part2 = (self._topics._topic_term[topic][word] + beta) / (sum(self._topics._topic_term[topic]) + len(vocab) * beta)
            sample_probs[kk] = part1 * part2
            # print("*************************************")
            # print(part1 * part2)
        return sample_probs
